...
> 💡 **访问优化建议**：若配合反代将局域网 IP 映射为自定义短域名（如 `http://chat.lan` 或 `http://l.com`），局域网内所有手机、电脑、平板打开浏览器即可直达，无需记忆IP。
...
* **唯一身份码生成**：首次进入系统自动分配一个 6 位唯一随机身份码。用户可在个人设置页随时将其修改为自定义身份码。
...
* **拖放式文件上传（桌面端）**：在 PC 上可将文件直接拖入消息输入框，自动加入待发送队列，确认后再发送；拖入文件夹会被自动忽略。
...
### 5. 隐蔽管理入口与后台运维 (`/admin`)
* **暗号式后台登录**：前端界面完全隐藏管理后台入口与按钮。管理员只需在**主聊天输入框内输入「自定义暗号」并直接发送**，系统自动完成鉴权并无缝跳转至 `/admin` 管理控制台。
* **全量数据审计与管理**：
...
* **可添加到桌面 / 主屏（PWA）**：内置 Web App Manifest 与多尺寸应用图标，Android Chrome、iOS Safari 与桌面浏览器均可将其安装为独立图标，全屏无浏览器界面运行。
