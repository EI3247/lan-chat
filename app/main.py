...
APP_VERSION = "202609032035"
APP_UPDATED_AT = "2026-09-03 20:35"
APP_CHANGELOG = [
    ...
    '附件菜单新增「上传图片」选项（配备图片线条 SVG，带 accept 过滤与多图选择），支持快捷单独选取相册图片。',
    '优化 📎 附件按钮尺寸与图标饱满度：加大点击热区与 SVG 视觉尺寸（从 20px 调大至 24px，线宽微调加粗至 2.0px），提升移动端触控体验。',
    '优化 📎 附件按钮交互：更换为现代回形针线条 SVG，新增按压与菜单展开时的旋转强调动画及二级菜单轻量弹入效果。',
    '输入框附件菜单新增「拍照上传」功能（移动端直接唤起相机），并将附件菜单图标升级为精致线条式 SVG 图标。',
    ...
    '文件信息弹窗与个人名片页 UI 重构：文件弹窗新增大图预览区+元信息卡片+「技术详情」折叠区（MIME/ID/URL 收起不再刷屏）；名片页资料改小卡片并去掉无意义字段。后台 /i 信息页精简为纯版本行式布局（移除变更列表与调试提示）。',
