...
async function openTextFile(fid){try{const d=await api(`/api/file/${fid}/text`);const dlg=$('#textFileDialog'),ta=$('#textFileContent'),code=$('#textFileCode');dlg.dataset.fid=fid;dlg.dataset.isOwner=d.is_owner?'1':'0';dlg.dataset.original=d.content;$('#textFileTitle').textContent=d.name;$('#textFileMeta').textContent=`${size(d.size)} · ${d.encoding}`;ta.value=d.content;const ext=(d.name||'').split('.').pop().toLowerCase();if(code)code.innerHTML=highlightCodeSyntax(d.content,ext);setTextFileMode('view');showDialog(dlg)}catch(e){toast('文本文件打开失败')}}
