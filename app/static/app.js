...
function openIdentityDialog(){const has=identityState.has_secret; $('#identityTitle').textContent=has?'修改身份码·密码':'设置身份码·密码'; $('#identityHint').textContent=has?'修改身份码或密码都需输入原密码验证。新密码留空=不改。':'身份码本身就是凭证，密码可选（留空=空密码，凭身份码即可恢复）。想更安全可设个密码。'; $('#idCodeInput').value=identityState.id_code||''; $('#oldSecretInput').value=''; $('#newSecretInput').value=''; $('#identityError').textContent=''; $('#oldSecretWrap').classList.toggle('hidden',!has); showDialog($('#identityDialog'))}
...
function openLightboxFromImg(imgEl, container){
  // 收集容器内所有可看图（img.zoomable）作为画廊，视频不是 img 自然排除；GIF/实况包含。
  ...
  openLightbox(list[idx]||imgEl.dataset.full||imgEl.src, document.body, false, list, idx);
}
function openLightbox(src, mount=document.body, round=false, gallery=null, gIndex=0){
  ...
  function go(dir){ // dir: +1 下一张 / -1 上一张
    ...
    const ni=gPos+dir; if(ni<0||ni>=gList.length) return; gPos=ni;
    const im=img(); reset();
    // 滑出动画：旧图滑走，新图滑入
    const outX=dir>0?'-60%':'60%', inFrom=dir>0?'60%':'-60%';
    im.style.transition='transform .16s ease, opacity .16s ease'; im.style.opacity='0'; im.style.transform=`translateX(${outX}) scale(.96)`;
    setTimeout(()=>{ im.src=gList[gPos]; im.style.transition='none'; im.style.opacity='0'; im.style.transform=`translateX(${inFrom}) scale(.96)`; requestAnimationFrame(()=>{ im.style.transition='transform .2s ease, opacity .2s ease'; zoom=1;x=0;y=0; im.style.opacity='1'; im.style.transform='translate(0px,0px) scale(1)'; setTimeout(()=>{im.style.transition='none'; apply();},210); }); updateCounter(); },170);
  }
  ...
  (mount||document.body).appendChild(box); apply();
}
